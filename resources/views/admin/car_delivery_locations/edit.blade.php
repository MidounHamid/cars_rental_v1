...
<div class="table-container">
    <h2>Edit Car Delivery Location</h2>
    <form action="{{ route('car_delivery_locations.update', $delivery->id) }}" method="POST" class="car-delivery-location-form">
        @csrf
        @method('PUT')
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="car_id" id="car_id" class="form-input" required>
                    @foreach($cars as $car)
                        <option value="{{ $car->id }}" {{ old('car_id', $delivery->car_id) == $car->id ? 'selected' : '' }}>
                            {{ $car->model }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-footer">
            <button type="submit" class="add-btn">Update Car Delivery Location</button>
