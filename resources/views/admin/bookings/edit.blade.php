    ...
    <form action="{{ route('bookings.update', $booking->id) }}" method="POST" class="booking-form">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" id="start_date" name="start_date" class="form-input" value="{{ old('start_date', $booking->start_date) }}">
                @error('start_date')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" id="end_date" name="end_date" class="form-input" value="{{ old('end_date', $booking->end_date) }}">
                @error('end_date')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" step="0.01" id="total_price" name="total_price" class="form-input" value="{{ old('total_price', $booking->total_price) }}">
                @error('total_price')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="promotion_id" name="promotion_id" class="form-input">
                    ...
                    @foreach($promotions as $promo)
                        <option value="{{ $promo->id }}" {{ old('promotion_id', $booking->promotion_id) == $promo->id ? 'selected' : '' }}>
